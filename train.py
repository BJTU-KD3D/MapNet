...
from collate_batch import collate_fn_with_sfc, collate_fn_with_multi_sfc
...
def get_logger():
    ...
    file_fmt = logging.Formatter("[%(asctime)s %(levelname)s %(filename)s line %(lineno)d %(process)d] %(message)s")
    file_handler.setFormatter(file_fmt)
    logger.addHandler(file_handler)
    
    console_handler = logging.StreamHandler()
    console_handler.setLevel(logging.INFO)
    console_fmt = logging.Formatter("[%(asctime)s %(levelname)s %(filename)s line %(lineno)d %(process)d] %(message)s")
...
def main():
    ...
    if args.multiprocessing_distributed:
        ...
        main_worker(args.train_gpu[0], 1, args)
...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    use_multi_space_filling = True
    sfc_methods = ["simple", "circular", "cylindrical"] 
    
    if use_multi_space_filling and main_process():
        logger.info(f"Using efficient space filling curve optimization, method: {sfc_methods}")
        
    def collate_multi_sfc(batch):
        return collate_fn_with_multi_sfc(batch, methods=sfc_methods)
    ...
    train_loader = torch.utils.data.DataLoader(
        ...
        drop_last=True, 
        collate_fn=collate_multi_sfc if use_multi_space_filling else collate_fn
    )
